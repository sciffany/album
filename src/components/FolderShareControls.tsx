...
export function FolderShareControls({
  ...
  const btn =
    compact
      ? "rounded-md border border-[var(--border)] px-2 py-1 text-xs text-[var(--ink)] transition hover:bg-[var(--surface-2)] disabled:opacity-50"
  ...
  function onCopy(e: MouseEvent) {
    ...
    startTransition(async () => {
      try {
        const result = await createOrGetFolderShareAction(path);
        if (!result.ok) {
  ...
  function onRevoke(e: MouseEvent) {
    ...
    startTransition(async () => {
      try {
        const result = await revokeFolderShareAction(path);
        if (!result.ok) {
          setError(result.error);
          return;
        }
